...
public static class Replicator {
    ...
    public static async Task Replicate(
        ...
        if(replicatorOptions.ReadFromEnd) {
            IPositionReader toReadFrom = replicatorOptions.ReadFromEndFrom == "writer"
                ? writer
                : reader;
            var checkpoint = await checkpointStore.LoadCheckpoint(linkedCts.Token).ConfigureAwait(false);
            var pos = await toReadFrom.GetLastFullPositionBasedOnPrevious(checkpoint, linkedCts.Token).ConfigureAwait(false);
            await checkpointStore.StoreCheckpoint(pos, linkedCts.Token).ConfigureAwait(false);
