...
public class TcpEventWriter(IEventStoreConnection connection) : IEventWriter {
    ...
    public async Task<long?> GetLastPosition(CancellationToken cancellationToken) {
        ...
        var last = await connection.ReadAllEventsBackwardAsync(Position.End, 1, false).ConfigureAwait(false);

        return last.NextPosition.CommitPosition;
    }

    public async Task<LogPosition> GetLastFullPosition(CancellationToken cancellationToken) {
        var last = await connection.ReadAllEventsBackwardAsync(Position.End, 1, false).ConfigureAwait(false);
        return new LogPosition(last.NextPosition.PreparePosition, (ulong)last.NextPosition.CommitPosition);
    }
    ...
    public Task<LogPosition> GetLastFullPositionBasedOnPrevious(LogPosition position, CancellationToken cancellationToken) => throw new NotImplementedException();
}
