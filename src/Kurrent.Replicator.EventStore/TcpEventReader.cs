...
public class TcpEventReader : IEventReader {
    ...
    public async Task<LogPosition> GetLastFullPositionBasedOnPrevious(LogPosition logPosition, CancellationToken cancellationToken) {
        if (!_connected) await _connection.ConnectAsync().ConfigureAwait(false);
